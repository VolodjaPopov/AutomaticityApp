...
test.describe("Logout and profile checking tests", () => {
  ...
  test(
    "Attempt to log out with invalid token",
    { tag: "@regression" },
    async ({ authAPI }) => {
      await authAPI.logout({
        token: INVALID_USER_CREDENTIALS["INVALID_TOKEN"],
        statusCode: 500,
  ...
  test(
    "Attempt to log out with expired token",
    { tag: "@regression" },
    async ({ authAPI }) => {
      await authAPI.logout({
  ...
  test(
    "Attempt to get profile of user with invalid token",
    { tag: "@regression" },
    async ({ authAPI }) => {
  ...
  test(
    "Attempt to fetch profile of recently deleted user",
    { tag: "@regression" },
